...
const useStringHelper = () => {
  ...
  const isKatakana = (value: string | null) => {
    value = value === null ? '' : value;
    // eslint-disable-next-line
    if (value.match(/^[ァ-ヶー]*$/)) {
      return true;
  ...
  const divideKanaKanji = (value: string) => {
  ...
  const dropRuby = (val: string) => {
    let replaced = val.replace(/《(.*?)》/g, '');
    replaced = replaced.replace(/〓/g, '');
    return replaced;
  };

  const convertRuby = (val: string) => {
    const boxes = [];
